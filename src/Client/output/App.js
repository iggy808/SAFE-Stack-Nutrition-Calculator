...
(function () {
    const program_3 = Program_Internal_withReactSynchronousUsing((equal, view, state, dispatch_1) => lazyView2With(uncurry2(equal), uncurry2(view), state, dispatch_1), "elmish-app", ProgramModule_withConsoleTrace(ProgramModule_mkProgram(init, update, view_1)));
    ...
    const mapUpdate = (userUpdate, msg_1, model_2) => {
        let patternInput;
        if (msg_1.tag === 1) {
            patternInput = [model_2, Cmd_none()];
        }
        else {
            const userMsg = msg_1.fields[0];
            patternInput = userUpdate(userMsg)(model_2);
        }
    ...
    const mapSubscribe = (subscribe, model_2_1) => Sub_batch(ofArray([Sub_map("HmrUser", (Item_3) => (new Msg$1(0, [Item_3])), subscribe(model_2_1)), hmrSubscription]));
    ...
    ProgramModule_runWith(undefined, ProgramModule_map(mapInit, mapUpdate, mapView, mapSetState, mapSubscribe, mapTermination, program_3));
})();
